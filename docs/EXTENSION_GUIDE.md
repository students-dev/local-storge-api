...
The Local Storage API VS Code extension provides comprehensive debugging, inspection, and management tools for client-side storage.
...
5. In the new window, open a project using Local Storage API

## Features

### Storage Explorer
Tree view sidebar showing all storage layers and their contents.

- **IndexedDB Databases**: Browse object stores and indexes
- **LocalStorage Items**: View key-value pairs
- **Memory Cache**: Inspect in-memory storage
- **Real-time Updates**: Live synchronization with storage changes

### Inspector Panel
Advanced inspection interface with multiple views.

- **Data Viewer**: Formatted display of complex objects
- **Type Information**: Data type analysis and statistics
- **Metadata Display**: Creation/update times, versions, TTL
- **Search & Filter**: Find specific keys or data patterns

### Metrics Dashboard
Performance monitoring and analytics.

- **Operation Counts**: Reads, writes, deletes over time
- **Latency Charts**: Response time graphs
- **Storage Usage**: Per-layer capacity utilization
- **Error Tracking**: Failed operations and error rates

### Snapshot Management
Time travel debugging and backup tools.
...
- **Snapshot Browser**: Timeline of all saved states

### Diff Viewer
Visual comparison of storage states.

- **Added Items**: New keys since last snapshot
- **Removed Items**: Deleted keys
- **Changed Items**: Modified values
...
2. Open Storage Explorer in sidebar
3. Click on storage items to inspect their values
4. Use search box to filter items
...
3. Compare current state with snapshot
4. Use diff viewer to identify changes
...
Main inspection interface with tabs for different views:
...
- Ensure Local Storage API is installed in project
...
### v1.0.0
- Initial release with core inspection features
- Storage explorer and metrics dashboard
- Snapshot management and diff viewer
- REPL terminal and migration tools
